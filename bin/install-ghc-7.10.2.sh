...
#
GHC_VER=7.10.2
CABAL_LIB_VER=1.22.4.0
CABAL_APP_VER=1.22.6.0
...
if [[ ! -f $TMP_DIR/ghc-$GHC_VER-x86_64-unknown-linux-deb7.tar.xz ]]; then
  mkdir -p $TMP_DIR
  cd $TMP_DIR && curl -L $GHC_URL > ghc-$GHC_VER-x86_64-unknown-linux-deb7.tar.xz
fi

# extract the tar file
if [[ ! -d $TMP_DIR/ghc-$GHC_VER ]]; then
  cd $TMP_DIR && tar -xvf ghc-$GHC_VER-x86_64-unknown-linux-deb7.tar.xz
fi
...
if [[ ! -f $CABAL_DIR/bin/cabal ]]; then
 cd $TMP_DIR/cabal-install-$CABAL_APP_VER && PREFIX=$CABAL_DIR ./bootstrap.sh
fi
