...
map <leader>P :exe '!clear && stack exec hello-debian -- +RTS -sstderr'<CR>
"" build for threadscope with eventlog
map <leader>e :exe '!clear && stack build --ghc-options="-eventlog -rtsopts"'<CR>
map <leader>E :exe '!clear && stack exec hello-debian -- +RTS -N2 -l'<CR>
"" build - build project
...
"" run - run program
map <leader>r :exe '!clear && stack build && clear && stack exec hello-debian'<CR>
"" spec - run specs
