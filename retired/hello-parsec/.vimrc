...
map <leader>P :exe '!clear && stack exec hello-parsec -- +RTS -sstderr'<CR>
"" build for threadscope with eventlog
map <leader>e :exe '!clear && stack build --ghc-options="-eventlog -rtsopts"'<CR>
map <leader>E :exe '!clear && stack exec hello-parsec -- +RTS -N2 -l'<CR>
"" build - build project
map <leader>b :exe '!clear && stack build --force-dirty'<CR>
"" clean - clean build artifacts
map <leader>c :exe '!clear && stack clean'<CR>
"" interactive - load modules in repl
map <leader>i :exe '!clear && stack ghci'<CR>
"" run - run program
map <leader>r :exe '!clear && stack build --force-dirty && clear && stack exec hello-parsec'<CR>
"" spec - run specs
